...
## 4.5.1 The Scheduling Decision (closes Finding #7)

Three real options exist for this stack, evaluated against the same evidence as everywhere else in this document — no new option was invented that doesn't already fit something this system or n8n natively supports:

| Option | Evidence-based assessment |
|---|---|
| **n8n's native Schedule Trigger node** | n8n is already the platform being stood up for Module 3; a Schedule Trigger is a built-in node type, no new infrastructure. Calls Gatepath through the exact `api_keys`/`/api/v1/*` layer every other n8n-initiated action in this plan already uses (§4.4) — one credential model, not two. |
| **Cloudflare Cron Triggers** (`wrangler.jsonc`'s `"triggers": { "crons": [...] }` + a `scheduled()` export in `src/server.ts`) | Native to this app's own hosting platform, zero new vendor relationship. But: this app has never had a `scheduled()` handler (confirmed — `src/server.ts` only exports `fetch`), and every schedule would live in a config file only a developer edits — the opposite of Module 3's own premise, which is giving staff/CEO a visual, editable view of what runs when. |
| **Supabase `pg_cron`** | Postgres-native, runs independent of both this app and n8n's own uptime. But: introduces a *second* scheduling primitive alongside n8n's, split across two systems with no shared view — directly contradicts this document's own Hybrid recommendation (§3.1) that money/signature/CEO-alert-adjacent flows need **one** coordinated, legible place, not two. |

**Decision: n8n's native Schedule Trigger is the primary and, for now, only scheduling mechanism for every Module 3 business-automation workflow** (overdue-installment dunning, escalation push, agent-score recompute, booking reminders — every "scheduled scan" named anywhere in §2/§4.6/§5.2). This keeps *when* something runs and *what* it does in the same place (the n8n canvas), consistent with why n8n was chosen as the automation layer in the first place, rather than splitting timing into a Gatepath config file only a developer can touch.

**Cloudflare Cron Triggers stay unused, reserved, not built now** — a `scheduled()` handler is real, low-effort work (a few lines in `server.ts` + one `wrangler.jsonc` field) *if and when* a future workflow needs guaranteed execution independent of n8n's own uptime. Nothing in the current roadmap needs that: even payment reconciliation (§4.6 Phase 2a/2b) runs synchronously at verify-time inside Gatepath's own server, not on a schedule at all. Building this speculatively now, with no workflow that needs it, would be exactly the kind of "adding infrastructure ahead of a real need" this whole engagement's evidence-first discipline argues against.

**`pg_cron` is explicitly not adopted** — not because it's a bad tool, but because running two independent schedulers (n8n's and Postgres's) for the same class of business workflow would recreate the coordination problem Architecture C (§3.1) was chosen specifically to avoid.

**What this means concretely for Phase 1 (WF-01, speed-to-lead) once approved**: no `scheduled()` handler, no `pg_cron` extension, no new `wrangler.jsonc` field. The SLA-escalation timer in WF-01 is an n8n Schedule Trigger workflow (e.g., every 15 minutes) that queries `/api/v1/leads` for anything unassigned/untouched past the SLA window and acts — entirely inside n8n, calling Gatepath only through the already-built, already-scoped API layer.
